...
- 9.1 Gems are rendered from their sprite images; there is no neon glow — the
  ...
  - 9.1.1 Each gem continuously spins in place — a lively idle rotation through its
    full turn, roughly one complete rotation every 1.5 seconds — while it rests on
    the board. Gems are not all in lockstep: their rotations are out of phase, so
    the board shimmers rather than pulsing as one. (This idle-spin pace is
    deliberately livelier than the slow transition pace of 9.3.)
